...
class MoneyTransferSearch extends AppMoneyTransfer {
    ...
    public function mySearch($params, $customerId) {
        $query = (new \yii\db\Query())
                ->select(["CONCAT('money-transfer+', money_transfer.id) AS id", 'money_transfer.r_customer', 'money_transfer.r_available_taemin', 'money_transfer.code', 'money_transfer.madmoun_name', 'money_transfer.remaining', 'money_transfer.updated_at', "CONCAT(customers.first_name, customers.fathers_name, customers.last_name) AS fullName", 'customers.first_name', 'customers.fathers_name', 'customers.last_name', 'available_taemin.name AS taemin_name'])
                ->from('money_transfer')
                ->join('LEFT JOIN', 'customers', 'customers.id = money_transfer.r_customer')
                ->join('LEFT JOIN', 'available_taemin', 'available_taemin.id = money_transfer.r_available_taemin');
