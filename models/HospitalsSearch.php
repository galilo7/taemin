...
//            [['fullName', 'taemin_name'], 'safe'],
    ...
    public function mySearch($params, $customerId) {
        $query = (new \yii\db\Query())
                ->select(["CONCAT('hospitals+', hospitals.id) AS id", 'hospitals.r_customer', 'hospitals.r_available_taemin', 'hospitals.code', 'hospitals.madmoun_name', 'hospitals.remaining', 'hospitals.updated_at', "CONCAT(customers.first_name, customers.fathers_name, customers.last_name) AS fullName", 'customers.first_name', 'customers.fathers_name', 'customers.last_name', 'available_taemin.name AS taemin_name'])
                ->from('hospitals')
                ->join('LEFT JOIN', 'customers', 'customers.id = hospitals.r_customer')
                ->join('LEFT JOIN', 'available_taemin', 'available_taemin.id = hospitals.r_available_taemin');
